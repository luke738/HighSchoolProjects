...
public class SystemSimulator
{
    ...
    public void populateSystem()
    {
        ...
        //bodies.add(new Rocket("rocket", 5*Math.pow(10, 5), 3.66, new Vector(400 * SystemSimulator.spaceCompression, 200 * SystemSimulator.spaceCompression), new Vector(745.8,0), new int[]{255, 255, 0}, 0*0.99*Math.pow(10,8), Math.pow(10,6)*Math.pow(10,10), 35000, 0));
    ...
    public void draw(Graphics2D g2)
    {
        for (Body body : bodies)
        {
            if(body.ident.equalsIgnoreCase("Rocket"))
            {
                g2.setColor(new Color(body.color[2], body.color[1], body.color[0]));
                g2.fill(new Ellipse2D.Double(SystemAnimator.B_WIDTH/2 - body.size/spaceCompression-3-(body.velocity.x/body.velocity.getMag())*10, SystemAnimator.B_HEIGHT/2-body.size/spaceCompression-3-(body.velocity.y/body.velocity.getMag())*10, ((body.size*2)/ spaceCompression)+6, ((body.size*2)/ spaceCompression)+6));
                g2.setColor(new Color(body.color[0], body.color[1], body.color[2]));
                g2.fill(new Ellipse2D.Double(SystemAnimator.B_WIDTH/2 - body.size/spaceCompression-5, SystemAnimator.B_HEIGHT/2-body.size/spaceCompression-5, ((body.size*2)/ spaceCompression)+10, ((body.size*2)/ spaceCompression)+10));
            }
            else
            {
                g2.setColor(new Color(body.color[0], body.color[1], body.color[2]));
                g2.fill(new Ellipse2D.Double((body.location.x-bodies.get(0).location.x-body.size)/ spaceCompression+SystemAnimator.B_WIDTH/2, (body.location.y-bodies.get(0).location.y-body.size)/ spaceCompression+SystemAnimator.B_HEIGHT/2, ((body.size*2)/ spaceCompression), ((body.size*2)/ spaceCompression)));
            }
        }
    }
